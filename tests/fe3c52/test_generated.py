...
class TestOtherModule:
    ...
    def test_index_html_title_not_empty_or_aaa(self):
        """测试 index.html 文件的 title 标签内容不为空且不是错误的 AAA"""
        index_file = Path("index.html")
        assert index_file.exists(), "index.html 文件不存在"
        ...
        assert title_content.upper() != "AAA", f"页面标题显示错误的 AAA，当前标题: {title_content}"
