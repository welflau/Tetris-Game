...
class TestOtherModule:
    ...
    def test_project_directory_structure(self):
        """测试项目目录结构是否正确"""
        # 检查根目录文件
        assert Path("index.html").exists(), "根目录缺少 index.html"
        
        # 检查文档目录结构
        docs_path = Path("docs")
        if docs_path.exists():
            fe3c52_path = docs_path / "fe3c52"
            if fe3c52_path.exists():
                dev_notes_path = fe3c52_path / "3ba7c2" / "dev-notes.md"
                assert dev_notes_path.exists(), "文档目录结构不完整"